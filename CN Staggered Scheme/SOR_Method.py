...
from math import exp,log,sqrt,cos,pi,sin,tan
...
def solve(h,dt):
    wopt = 2.0/(1 + sqrt(1.0 - (cos(pi*h))**2))
    iter = int(1/dt)
    t = 0
    l = dt/(2*h**2) 
        ...
        while(ratio > 1e-7):
            ...
            for i in range (1,nx+1):
                for j in range (1,ny+1):
                    ...
                    i1[i,j] = i1[i,j] + wopt * ( -i1[i,j] + (l/beta) * (i1[i-1,j] + i1[i+1,j] + i1[i,j+1] + i1[i,j-1]) + (1/beta) * rhs_i[i,j])
        ...
        while(ratio > 1e-7):
            ...
            for i in range (1,nx+1):
                for j in range (1,ny+1):
                    ...
                    s1[i,j] = s1[i,j] + wopt * ( -s1[i,j] + (l/alpha) * (s1[i-1,j] + s1[i+1,j] + s1[i,j+1] + s1[i,j-1]) + (1/alpha) * rhs_s[i,j])
...
if __name__ == "__main__":
    ...
    save_file = open('SOR_result.csv','w')
